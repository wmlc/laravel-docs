# Laravel MCP

- [简介](#introduction)
- [安装](#installation)
    - [发布路由](#publishing-routes)
- [创建服务器](#creating-servers)
    - [注册服务器](#server-registration)
    - [Web 服务器](#web-servers)
    - [本地服务器](#local-servers)
- [工具](#tools)
    - [创建工具](#creating-tools)
    - [工具输入 Schema](#tool-input-schemas)
    - [工具输出 Schema](#tool-output-schemas)
    - [验证工具参数](#validating-tool-arguments)
    - [工具依赖注入](#tool-dependency-injection)
    - [工具注解](#tool-annotations)
    - [条件注册工具](#conditional-tool-registration)
    - [工具响应](#tool-responses)
- [提示词](#prompts)
    - [创建提示词](#creating-prompts)
    - [提示词参数](#prompt-arguments)
    - [验证提示词参数](#validating-prompt-arguments)
    - [提示词依赖注入](#prompt-dependency-injection)
    - [条件注册提示词](#conditional-prompt-registration)
    - [提示词响应](#prompt-responses)
- [资源](#resources)
    - [创建资源](#creating-resources)
    - [资源模板](#resource-templates)
    - [资源 URI 与 MIME 类型](#resource-uri-and-mime-type)
    - [资源请求](#resource-request)
    - [资源依赖注入](#resource-dependency-injection)
    - [资源注解](#resource-annotations)
    - [条件注册资源](#conditional-resource-registration)
    - [资源响应](#resource-responses)
- [元数据](#metadata)
- [身份认证](#authentication)
    - [OAuth 2.1](#oauth)
    - [Sanctum](#sanctum)
- [授权](#authorization)
- [测试服务器](#testing-servers)
    - [MCP Inspector](#mcp-inspector)
    - [单元测试](#unit-tests)

<a name="introduction"></a>
## 简介

[Laravel MCP](https://github.com/laravel/mcp) 提供了一种简单优雅的方式，让 AI 客户端能够通过[模型上下文协议](https://modelcontextprotocol.io/docs/getting-started/intro)（Model Context Protocol）与你的 Laravel 应用交互。它为定义服务器、工具、资源和提示词提供了富表现力、流式的接口，从而实现 AI 驱动的应用交互。

<a name="installation"></a>
## 安装

开始之前，先使用 Composer 包管理器将 Laravel MCP 安装到你的项目中：

```shell
composer require laravel/mcp
```

<a name="publishing-routes"></a>
### 发布路由

安装 Laravel MCP 之后，执行 `vendor:publish` Artisan 命令来发布 `routes/ai.php` 文件，你将在该文件中定义 MCP 服务器：

```shell
php artisan vendor:publish --tag=ai-routes
```

该命令会在应用的 `routes` 目录中创建 `routes/ai.php` 文件，你将用它来注册 MCP 服务器。

<a name="creating-servers"></a>
## 创建服务器

你可以使用 `make:mcp-server` Artisan 命令创建 MCP 服务器。服务器作为中央通信节点，向 AI 客户端暴露工具、资源和提示词等 MCP 能力：

```shell
php artisan make:mcp-server WeatherServer
```

该命令会在 `app/Mcp/Servers` 目录中创建一个新的服务器类。生成的服务器类继承 Laravel MCP 的基类 `Laravel\Mcp\Server`，并提供用于配置服务器以及注册工具、资源和提示词的属性（Attribute）和属性（Property）：

```php
<?php

namespace App\Mcp\Servers;

use Laravel\Mcp\Server\Attributes\Instructions;
use Laravel\Mcp\Server\Attributes\Name;
use Laravel\Mcp\Server\Attributes\Version;
use Laravel\Mcp\Server;

#[Name('Weather Server')]
#[Version('1.0.0')]
#[Instructions('This server provides weather information and forecasts.')]
class WeatherServer extends Server
{
    /**
     * 注册到此 MCP 服务器的工具。
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Tool>>
     */
    protected array $tools = [
        // GetCurrentWeatherTool::class,
    ];

    /**
     * 注册到此 MCP 服务器的资源。
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Resource>>
     */
    protected array $resources = [
        // WeatherGuidelinesResource::class,
    ];

    /**
     * 注册到此 MCP 服务器的提示词。
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Prompt>>
     */
    protected array $prompts = [
        // DescribeWeatherPrompt::class,
    ];
}
```

<a name="server-registration"></a>
### 注册服务器

创建服务器后，必须在 `routes/ai.php` 文件中注册它，才能使其可被访问。Laravel MCP 提供了两种注册服务器的方法：`web` 用于可通过 HTTP 访问的服务器，`local` 用于命令行服务器。

<a name="web-servers"></a>
### Web 服务器

Web 服务器是最常见的服务器类型，可通过 HTTP POST 请求访问，非常适合远程 AI 客户端或基于 Web 的集成。使用 `web` 方法注册 Web 服务器：

```php
use App\Mcp\Servers\WeatherServer;
use Laravel\Mcp\Facades\Mcp;

Mcp::web('/mcp/weather', WeatherServer::class);
```

与普通路由一样，你可以应用中间件来保护 Web 服务器：

```php
Mcp::web('/mcp/weather', WeatherServer::class)
    ->middleware(['throttle:mcp']);
```

<a name="local-servers"></a>
### 本地服务器

本地服务器以 Artisan 命令的形式运行，非常适合构建本地 AI 助手集成，例如 [Laravel Boost](/docs/{{version}}/installation#installing-laravel-boost)。使用 `local` 方法注册本地服务器：

```php
use App\Mcp\Servers\WeatherServer;
use Laravel\Mcp\Facades\Mcp;

Mcp::local('weather', WeatherServer::class);
```

注册之后，你通常不需要自己手动运行 `mcp:start` Artisan 命令。相反，应配置你的 MCP 客户端（AI 代理）来启动服务器，或者使用 [MCP Inspector](#mcp-inspector)。

<a name="tools"></a>
## 工具

工具让你的服务器能够暴露可供 AI 客户端调用的功能。它们允许语言模型执行操作、运行代码或与外部系统交互：

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Tool;

#[Description('Fetches the current weather forecast for a specified location.')]
class CurrentWeatherTool extends Tool
{
    /**
     * 处理工具请求。
     */
    public function handle(Request $request): Response
    {
        $location = $request->get('location');

        // 获取天气...

        return Response::text('The weather is...');
    }

    /**
     * 获取工具的输入 schema。
     *
     * @return array<string, \Illuminate\JsonSchema\Types\Type>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'location' => $schema->string()
                ->description('The location to get the weather for.')
                ->required(),
        ];
    }
}
```

<a name="creating-tools"></a>
### 创建工具

要创建工具，请运行 `make:mcp-tool` Artisan 命令：

```shell
php artisan make:mcp-tool CurrentWeatherTool
```

创建工具后，在服务器的 `$tools` 属性中注册它：

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Tools\CurrentWeatherTool;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * 注册到此 MCP 服务器的工具。
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Tool>>
     */
    protected array $tools = [
        CurrentWeatherTool::class,
    ];
}
```

<a name="tool-name-title-description"></a>
#### 工具名称、标题与描述

默认情况下，工具的名称和标题由类名推导而来。例如，`CurrentWeatherTool` 的名称为 `current-weather`，标题为 `Current Weather Tool`。你可以使用 `Name` 和 `Title` 属性（Attribute）自定义这些值：

```php
use Laravel\Mcp\Server\Attributes\Name;
use Laravel\Mcp\Server\Attributes\Title;

#[Name('get-optimistic-weather')]
#[Title('Get Optimistic Weather Forecast')]
class CurrentWeatherTool extends Tool
{
    // ...
}
```

工具的描述不会自动生成。你应始终使用 `Description` 属性提供有意义的描述：

```php
use Laravel\Mcp\Server\Attributes\Description;

#[Description('Fetches the current weather forecast for a specified location.')]
class CurrentWeatherTool extends Tool
{
    //
}
```

> [!NOTE]
> 描述是工具元数据的关键部分，它能帮助 AI 模型理解何时以及如何有效地使用该工具。

<a name="tool-input-schemas"></a>
### 工具输入 Schema

工具可以定义输入 schema，来指定它接受来自 AI 客户端的哪些参数。使用 Laravel 的 `Illuminate\Contracts\JsonSchema\JsonSchema` 构建器来定义工具的输入要求：

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 获取工具的输入 schema。
     *
     * @return array<string, \Illuminate\JsonSchema\Types\Type>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'location' => $schema->string()
                ->description('The location to get the weather for.')
                ->required(),

            'units' => $schema->string()
                ->enum(['celsius', 'fahrenheit'])
                ->description('The temperature units to use.')
                ->default('celsius'),
        ];
    }
}
```

<a name="tool-output-schemas"></a>
### 工具输出 Schema

工具可以定义[输出 schema](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#output-schema)来指定响应的结构。这使得需要可解析工具结果的 AI 客户端能够更好地集成。使用 `outputSchema` 方法定义工具的输出结构：

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 获取工具的输出 schema。
     *
     * @return array<string, \Illuminate\JsonSchema\Types\Type>
     */
    public function outputSchema(JsonSchema $schema): array
    {
        return [
            'temperature' => $schema->number()
                ->description('Temperature in Celsius')
                ->required(),

            'conditions' => $schema->string()
                ->description('Weather conditions')
                ->required(),

            'humidity' => $schema->integer()
                ->description('Humidity percentage')
                ->required(),
        ];
    }
}
```

<a name="validating-tool-arguments"></a>
### 验证工具参数

JSON Schema 定义为工具参数提供了基本结构，但你可能还想强制执行更复杂的验证规则。

Laravel MCP 与 Laravel 的[验证功能](/docs/{{version}}/validation)无缝集成。你可以在工具的 `handle` 方法中验证传入的工具参数：

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 处理工具请求。
     */
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            'location' => 'required|string|max:100',
            'units' => 'in:celsius,fahrenheit',
        ]);

        // 使用验证后的参数获取天气数据...
    }
}
```

验证失败时，AI 客户端会根据你提供的错误消息采取行动。因此，提供清晰、可操作的错误消息至关重要：

```php
$validated = $request->validate([
    'location' => ['required','string','max:100'],
    'units' => 'in:celsius,fahrenheit',
],[
    'location.required' => 'You must specify a location to get the weather for. For example, "New York City" or "Tokyo".',
    'units.in' => 'You must specify either "celsius" or "fahrenheit" for the units.',
]);
```

<a name="tool-dependency-injection"></a>
#### 工具依赖注入

Laravel [服务容器（Service Container）](/docs/{{version}}/container)用于解析所有工具。因此，你可以在工具的构造函数中类型提示（type-hint）所需的任何依赖。声明的依赖会被自动解析并注入到工具实例中：

```php
<?php

namespace App\Mcp\Tools;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 创建新的工具实例。
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    // ...
}
```

除构造函数注入外，你还可以在工具的 `handle()` 方法中类型提示依赖。方法被调用时，服务容器会自动解析并注入这些依赖：

```php
<?php

namespace App\Mcp\Tools;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 处理工具请求。
     */
    public function handle(Request $request, WeatherRepository $weather): Response
    {
        $location = $request->get('location');

        $forecast = $weather->getForecastFor($location);

        // ...
    }
}
```

<a name="tool-annotations"></a>
### 工具注解

你可以通过[注解](https://modelcontextprotocol.io/specification/2025-06-18/schema#toolannotations)为工具增强功能，向 AI 客户端提供额外的元数据。这些注解有助于 AI 模型理解工具的行为和能力。注解通过属性（Attribute）添加到工具上：

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Server\Tools\Annotations\IsIdempotent;
use Laravel\Mcp\Server\Tools\Annotations\IsReadOnly;
use Laravel\Mcp\Server\Tool;

#[IsIdempotent]
#[IsReadOnly]
class CurrentWeatherTool extends Tool
{
    //
}
```

可用的注解包括：

| 注解 | 类型 | 说明 |
| --- | --- | --- |
| `#[IsReadOnly]` | boolean | 指示工具不会修改其环境。 |
| `#[IsDestructive]` | boolean | 指示工具可能执行破坏性更新（仅在非只读时有意义）。 |
| `#[IsIdempotent]` | boolean | 指示使用相同参数重复调用不会产生额外影响（非只读时）。 |
| `#[IsOpenWorld]` | boolean | 指示工具可能与外部实体交互。 |

注解的值可以通过布尔参数显式设置：

```php
use Laravel\Mcp\Server\Tools\Annotations\IsReadOnly;
use Laravel\Mcp\Server\Tools\Annotations\IsDestructive;
use Laravel\Mcp\Server\Tools\Annotations\IsOpenWorld;
use Laravel\Mcp\Server\Tools\Annotations\IsIdempotent;
use Laravel\Mcp\Server\Tool;

#[IsReadOnly(true)]
#[IsDestructive(false)]
#[IsOpenWorld(false)]
#[IsIdempotent(true)]
class CurrentWeatherTool extends Tool
{
    //
}
```

<a name="conditional-tool-registration"></a>
### 条件注册工具

你可以通过在工具类中实现 `shouldRegister` 方法，在运行时按条件注册工具。该方法允许你根据应用状态、配置或请求参数来决定工具是否可用：

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 判断是否应注册此工具。
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

当工具的 `shouldRegister` 方法返回 `false` 时，该工具不会出现在可用工具列表中，AI 客户端也无法调用它。

<a name="tool-responses"></a>
### 工具响应

工具必须返回一个 `Laravel\Mcp\Response` 实例。Response 类提供了多个便捷方法，用于创建不同类型的响应。

对于简单的文本响应，使用 `text` 方法：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 处理工具请求。
 */
public function handle(Request $request): Response
{
    // ...

    return Response::text('Weather Summary: Sunny, 72°F');
}
```

要指示工具执行过程中发生了错误，使用 `error` 方法：

```php
return Response::error('Unable to fetch weather data. Please try again.');
```

要返回图片或音频内容，使用 `image` 和 `audio` 方法：

```php
return Response::image(file_get_contents(storage_path('weather/radar.png')), 'image/png');

return Response::audio(file_get_contents(storage_path('weather/alert.mp3')), 'audio/mp3');
```

你还可以使用 `fromStorage` 方法直接从 Laravel 文件系统磁盘加载图片和音频内容。MIME 类型将根据文件自动检测：

```php
return Response::fromStorage('weather/radar.png');
```

必要时，你可以指定特定磁盘或覆盖 MIME 类型：

```php
return Response::fromStorage('weather/radar.png', disk: 's3');

return Response::fromStorage('weather/radar.png', mimeType: 'image/webp');
```

<a name="multiple-content-responses"></a>
#### 多内容响应

工具可以通过返回一个 `Response` 实例数组来返回多条内容：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 处理工具请求。
 *
 * @return array<int, \Laravel\Mcp\Response>
 */
public function handle(Request $request): array
{
    // ...

    return [
        Response::text('Weather Summary: Sunny, 72°F'),
        Response::text('**Detailed Forecast**\n- Morning: 65°F\n- Afternoon: 78°F\n- Evening: 70°F')
    ];
}
```

<a name="structured-responses"></a>
#### 结构化响应

工具可以使用 `structured` 方法返回[结构化内容](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#structured-content)。这为 AI 客户端提供了可解析的数据，同时保持与 JSON 编码文本表示形式的向后兼容：

```php
return Response::structured([
    'temperature' => 22.5,
    'conditions' => 'Partly cloudy',
    'humidity' => 65,
]);
```

如果需要在结构化内容之外提供自定义文本，可以在响应工厂上使用 `withStructuredContent` 方法：

```php
return Response::make(
    Response::text('Weather is 22.5°C and sunny')
)->withStructuredContent([
    'temperature' => 22.5,
    'conditions' => 'Sunny',
]);
```

<a name="streaming-responses"></a>
#### 流式响应

对于长时间运行的操作或实时数据流，工具可以在其 `handle` 方法中返回一个[生成器](https://www.php.net/manual/en/language.generators.overview.php)。这样就可以在最终响应之前向客户端发送中间更新：

```php
<?php

namespace App\Mcp\Tools;

use Generator;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * 处理工具请求。
     *
     * @return \Generator<int, \Laravel\Mcp\Response>
     */
    public function handle(Request $request): Generator
    {
        $locations = $request->array('locations');

        foreach ($locations as $index => $location) {
            yield Response::notification('processing/progress', [
                'current' => $index + 1,
                'total' => count($locations),
                'location' => $location,
            ]);

            yield Response::text($this->forecastFor($location));
        }
    }
}
```

使用基于 Web 的服务器时，流式响应会自动开启 SSE（Server-Sent Events）流，将每条 yield 的消息作为一个事件发送给客户端。

<a name="prompts"></a>
## 提示词

[提示词](https://modelcontextprotocol.io/specification/2025-06-18/server/prompts)让你的服务器能够共享可复用的提示词模板，供 AI 客户端在与语言模型交互时使用。它们为构建常见的查询和交互提供了一种标准化的方式。

<a name="creating-prompts"></a>
### 创建提示词

要创建提示词，请运行 `make:mcp-prompt` Artisan 命令：

```shell
php artisan make:mcp-prompt DescribeWeatherPrompt
```

创建提示词后，在服务器的 `$prompts` 属性中注册它：

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Prompts\DescribeWeatherPrompt;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * 注册到此 MCP 服务器的提示词。
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Prompt>>
     */
    protected array $prompts = [
        DescribeWeatherPrompt::class,
    ];
}
```

<a name="prompt-name-title-and-description"></a>
#### 提示词名称、标题与描述

默认情况下，提示词的名称和标题由类名推导而来。例如，`DescribeWeatherPrompt` 的名称为 `describe-weather`，标题为 `Describe Weather Prompt`。你可以使用 `Name` 和 `Title` 属性自定义这些值：

```php
use Laravel\Mcp\Server\Attributes\Name;
use Laravel\Mcp\Server\Attributes\Title;

#[Name('weather-assistant')]
#[Title('Weather Assistant Prompt')]
class DescribeWeatherPrompt extends Prompt
{
    // ...
}
```

提示词的描述不会自动生成。你应始终使用 `Description` 属性提供有意义的描述：

```php
use Laravel\Mcp\Server\Attributes\Description;

#[Description('Generates a natural-language explanation of the weather for a given location.')]
class DescribeWeatherPrompt extends Prompt
{
    //
}
```

> [!NOTE]
> 描述是提示词元数据的关键部分，它能帮助 AI 模型理解何时以及如何充分发挥该提示词的作用。

<a name="prompt-arguments"></a>
### 提示词参数

提示词可以定义参数，允许 AI 客户端用特定的值自定义提示词模板。使用 `arguments` 方法定义提示词接受哪些参数：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Server\Prompt;
use Laravel\Mcp\Server\Prompts\Argument;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 获取提示词的参数。
     *
     * @return array<int, \Laravel\Mcp\Server\Prompts\Argument>
     */
    public function arguments(): array
    {
        return [
            new Argument(
                name: 'tone',
                description: 'The tone to use in the weather description (e.g., formal, casual, humorous).',
                required: true,
            ),
        ];
    }
}
```

<a name="validating-prompt-arguments"></a>
### 验证提示词参数

提示词参数会根据其定义自动验证，但你可能还想强制执行更复杂的验证规则。

Laravel MCP 与 Laravel 的[验证功能](/docs/{{version}}/validation)无缝集成。你可以在提示词的 `handle` 方法中验证传入的提示词参数：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 处理提示词请求。
     */
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            'tone' => 'required|string|max:50',
        ]);

        $tone = $validated['tone'];

        // 使用给定的语气生成提示词响应...
    }
}
```

验证失败时，AI 客户端会根据你提供的错误消息采取行动。因此，提供清晰、可操作的错误消息至关重要：

```php
$validated = $request->validate([
    'tone' => ['required','string','max:50'],
],[
    'tone.*' => 'You must specify a tone for the weather description. Examples include "formal", "casual", or "humorous".',
]);
```

<a name="prompt-dependency-injection"></a>
### 提示词依赖注入

Laravel [服务容器](/docs/{{version}}/container)用于解析所有提示词。因此，你可以在提示词的构造函数中类型提示所需的任何依赖。声明的依赖会被自动解析并注入到提示词实例中：

```php
<?php

namespace App\Mcp\Prompts;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 创建新的提示词实例。
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    //
}
```

除构造函数注入外，你还可以在提示词的 `handle` 方法中类型提示依赖。方法被调用时，服务容器会自动解析并注入这些依赖：

```php
<?php

namespace App\Mcp\Prompts;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 处理提示词请求。
     */
    public function handle(Request $request, WeatherRepository $weather): Response
    {
        $isAvailable = $weather->isServiceAvailable();

        // ...
    }
}
```

<a name="conditional-prompt-registration"></a>
### 条件注册提示词

你可以通过在提示词类中实现 `shouldRegister` 方法，在运行时按条件注册提示词。该方法允许你根据应用状态、配置或请求参数来决定提示词是否可用：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Prompt;

class CurrentWeatherPrompt extends Prompt
{
    /**
     * 判断是否应注册此提示词。
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

当提示词的 `shouldRegister` 方法返回 `false` 时，该提示词不会出现在可用提示词列表中，AI 客户端也无法调用它。

<a name="prompt-responses"></a>
### 提示词响应

提示词可以返回单个 `Laravel\Mcp\Response`，或一个可迭代的 `Laravel\Mcp\Response` 实例集合。这些响应封装了将发送给 AI 客户端的内容：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * 处理提示词请求。
     *
     * @return array<int, \Laravel\Mcp\Response>
     */
    public function handle(Request $request): array
    {
        $tone = $request->string('tone');

        $systemMessage = "You are a helpful weather assistant. Please provide a weather description in a {$tone} tone.";

        $userMessage = "What is the current weather like in New York City?";

        return [
            Response::text($systemMessage)->asAssistant(),
            Response::text($userMessage),
        ];
    }
}
```

你可以使用 `asAssistant()` 方法来指示某条响应消息应被视为来自 AI 助手的消息，而普通消息则被视为用户输入。

<a name="resources"></a>
## 资源

[资源](https://modelcontextprotocol.io/specification/2025-06-18/server/resources)让你的服务器能够暴露供 AI 客户端读取的数据和内容，并在与语言模型交互时将其用作上下文。它们提供了一种共享静态或动态信息的方式，例如文档、配置或任何有助于为 AI 响应提供依据的数据。

<a name="creating-resources"></a>
## 创建资源

要创建资源，请运行 `make:mcp-resource` Artisan 命令：

```shell
php artisan make:mcp-resource WeatherGuidelinesResource
```

创建资源后，在服务器的 `$resources` 属性中注册它：

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Resources\WeatherGuidelinesResource;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * 注册到此 MCP 服务器的资源。
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Resource>>
     */
    protected array $resources = [
        WeatherGuidelinesResource::class,
    ];
}
```

<a name="resource-name-title-and-description"></a>
#### 资源名称、标题与描述

默认情况下，资源的名称和标题由类名推导而来。例如，`WeatherGuidelinesResource` 的名称为 `weather-guidelines`，标题为 `Weather Guidelines Resource`。你可以使用 `Name` 和 `Title` 属性自定义这些值：

```php
use Laravel\Mcp\Server\Attributes\Name;
use Laravel\Mcp\Server\Attributes\Title;

#[Name('weather-api-docs')]
#[Title('Weather API Documentation')]
class WeatherGuidelinesResource extends Resource
{
    // ...
}
```

资源的描述不会自动生成。你应始终使用 `Description` 属性提供有意义的描述：

```php
use Laravel\Mcp\Server\Attributes\Description;

#[Description('Comprehensive guidelines for using the Weather API.')]
class WeatherGuidelinesResource extends Resource
{
    //
}
```

> [!NOTE]
> 描述是资源元数据的关键部分，它能帮助 AI 模型理解何时以及如何有效地使用该资源。

<a name="resource-templates"></a>
### 资源模板

[资源模板](https://modelcontextprotocol.io/specification/2025-06-18/server/resources#resource-templates)让你的服务器能够暴露动态资源，通过带变量的 URI 模式进行匹配。你无需为每个资源定义静态 URI，而是可以创建单个资源，基于模板模式处理多个 URI。

<a name="creating-resource-templates"></a>
#### 创建资源模板

要创建资源模板，需在资源类上实现 `HasUriTemplate` 接口，并定义一个返回 `UriTemplate` 实例的 `uriTemplate` 方法：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Attributes\MimeType;
use Laravel\Mcp\Server\Contracts\HasUriTemplate;
use Laravel\Mcp\Server\Resource;
use Laravel\Mcp\Support\UriTemplate;

#[Description('Access user files by ID')]
#[MimeType('text/plain')]
class UserFileResource extends Resource implements HasUriTemplate
{
    /**
     * 获取此资源的 URI 模板。
     */
    public function uriTemplate(): UriTemplate
    {
        return new UriTemplate('file://users/{userId}/files/{fileId}');
    }

    /**
     * 处理资源请求。
     */
    public function handle(Request $request): Response
    {
        $userId = $request->get('userId');
        $fileId = $request->get('fileId');

        // 获取并返回文件内容...

        return Response::text($content);
    }
}
```

当资源实现了 `HasUriTemplate` 接口时，它将被注册为资源模板，而非静态资源。之后，AI 客户端可以使用与模板模式匹配的 URI 请求资源，URI 中的变量会被自动提取，并可在资源的 `handle` 方法中使用。

<a name="uri-template-syntax"></a>
#### URI 模板语法

URI 模板使用花括号包裹的占位符来定义 URI 中的可变部分：

```php
new UriTemplate('file://users/{userId}');
new UriTemplate('file://users/{userId}/files/{fileId}');
new UriTemplate('https://api.example.com/{version}/{resource}/{id}');
```

<a name="accessing-template-variables"></a>
#### 访问模板变量

当 URI 与你的资源模板匹配时，提取出的变量会自动合并到请求中，可以通过 `get` 方法访问：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Contracts\HasUriTemplate;
use Laravel\Mcp\Server\Resource;
use Laravel\Mcp\Support\UriTemplate;

class UserProfileResource extends Resource implements HasUriTemplate
{
    public function uriTemplate(): UriTemplate
    {
        return new UriTemplate('file://users/{userId}/profile');
    }

    public function handle(Request $request): Response
    {
        // 访问提取出的变量
        $userId = $request->get('userId');

        // 必要时访问完整 URI
        $uri = $request->uri();

        // 获取用户资料...

        return Response::text("Profile for user {$userId}");
    }
}
```

`Request` 对象既提供提取出的变量，也提供所请求的原始 URI，为你处理资源请求提供完整的上下文。

<a name="resource-uri-and-mime-type"></a>
### 资源 URI 与 MIME 类型

每个资源都由唯一的 URI 标识，并带有关联的 MIME 类型，帮助 AI 客户端理解资源的格式。

默认情况下，资源的 URI 根据资源名称生成，因此 `WeatherGuidelinesResource` 的 URI 为 `weather://resources/weather-guidelines`。默认 MIME 类型为 `text/plain`。

你可以使用 `Uri` 和 `MimeType` 属性自定义这些值：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Server\Attributes\MimeType;
use Laravel\Mcp\Server\Attributes\Uri;
use Laravel\Mcp\Server\Resource;

#[Uri('weather://resources/guidelines')]
#[MimeType('application/pdf')]
class WeatherGuidelinesResource extends Resource
{
}
```

URI 和 MIME 类型帮助 AI 客户端确定如何正确处理和解读资源内容。

<a name="resource-request"></a>
### 资源请求

与工具和提示词不同，资源无法定义输入 schema 或参数。不过，你仍然可以在资源的 `handle` 方法中与请求对象交互：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 处理资源请求。
     */
    public function handle(Request $request): Response
    {
        // ...
    }
}
```

<a name="resource-dependency-injection"></a>
### 资源依赖注入

Laravel [服务容器](/docs/{{version}}/container)用于解析所有资源。因此，你可以在资源的构造函数中类型提示所需的任何依赖。声明的依赖会被自动解析并注入到资源实例中：

```php
<?php

namespace App\Mcp\Resources;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 创建新的资源实例。
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    // ...
}
```

除构造函数注入外，你还可以在资源的 `handle` 方法中类型提示依赖。方法被调用时，服务容器会自动解析并注入这些依赖：

```php
<?php

namespace App\Mcp\Resources;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 处理资源请求。
     */
    public function handle(WeatherRepository $weather): Response
    {
        $guidelines = $weather->guidelines();

        return Response::text($guidelines);
    }
}
```

<a name="resource-annotations"></a>
### 资源注解

你可以通过[注解](https://modelcontextprotocol.io/specification/2025-06-18/schema#resourceannotations)为资源增强功能，向 AI 客户端提供额外的元数据。注解通过属性（Attribute）添加到资源上：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Enums\Role;
use Laravel\Mcp\Server\Annotations\Audience;
use Laravel\Mcp\Server\Annotations\LastModified;
use Laravel\Mcp\Server\Annotations\Priority;
use Laravel\Mcp\Server\Resource;

#[Audience(Role::User)]
#[LastModified('2025-01-12T15:00:58Z')]
#[Priority(0.9)]
class UserDashboardResource extends Resource
{
    //
}
```

可用的注解包括：

| 注解 | 类型 | 说明 |
| --- | --- | --- |
| `#[Audience]` | Role 或数组 | 指定目标受众（`Role::User`、`Role::Assistant` 或两者）。 |
| `#[Priority]` | float | 介于 0.0 和 1.0 之间的数值分数，表示资源的重要性。 |
| `#[LastModified]` | string | ISO 8601 时间戳，表示资源最近一次更新的时间。 |

<a name="conditional-resource-registration"></a>
### 条件注册资源

你可以通过在资源类中实现 `shouldRegister` 方法，在运行时按条件注册资源。该方法允许你根据应用状态、配置或请求参数来决定资源是否可用：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * 判断是否应注册此资源。
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

当资源的 `shouldRegister` 方法返回 `false` 时，该资源不会出现在可用资源列表中，AI 客户端也无法访问它。

<a name="resource-responses"></a>
### 资源响应

资源必须返回一个 `Laravel\Mcp\Response` 实例。Response 类提供了多个便捷方法，用于创建不同类型的响应。

对于简单的文本内容，使用 `text` 方法：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 处理资源请求。
 */
public function handle(Request $request): Response
{
    // ...

    return Response::text($weatherData);
}
```

<a name="resource-blob-responses"></a>
#### Blob 响应

要返回 blob 内容，使用 `blob` 方法并提供 blob 内容：

```php
return Response::blob(file_get_contents(storage_path('weather/radar.png')));
```

返回 blob 内容时，MIME 类型由资源配置的 MIME 类型决定：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Server\Attributes\MimeType;
use Laravel\Mcp\Server\Resource;

#[MimeType('image/png')]
class WeatherGuidelinesResource extends Resource
{
    //
}
```

<a name="resource-error-responses"></a>
#### 错误响应

要指示资源获取过程中发生了错误，使用 `error()` 方法：

```php
return Response::error('Unable to fetch weather data for the specified location.');
```

<a name="metadata"></a>
## 元数据

Laravel MCP 还支持 [MCP 规范](https://modelcontextprotocol.io/specification/2025-06-18/basic#meta)中定义的 `_meta` 字段，某些 MCP 客户端或集成会要求该字段。元数据可以应用于所有 MCP 原语，包括工具、资源和提示词，以及它们的响应。

你可以使用 `withMeta` 方法将元数据附加到单个响应内容上：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 处理工具请求。
 */
public function handle(Request $request): Response
{
    return Response::text('The weather is sunny.')
        ->withMeta(['source' => 'weather-api', 'cached' => true]);
}
```

对于作用于整个响应信封（envelope）的结果级元数据，请用 `Response::make` 包装你的响应，并在返回的响应工厂实例上调用 `withMeta`：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\ResponseFactory;

/**
 * 处理工具请求。
 */
public function handle(Request $request): ResponseFactory
{
    return Response::make(
        Response::text('The weather is sunny.')
    )->withMeta(['request_id' => '12345']);
}
```

要将元数据附加到工具、资源或提示词本身，可以在类上定义 `$meta` 属性：

```php
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Tool;

#[Description('Fetches the current weather forecast.')]
class CurrentWeatherTool extends Tool
{
    protected ?array $meta = [
        'version' => '2.0',
        'author' => 'Weather Team',
    ];

    // ...
}
```

<a name="authentication"></a>
## 身份认证

与路由一样，你可以通过中间件对 Web MCP 服务器进行身份认证。为 MCP 服务器添加身份认证后，用户需要先通过认证，然后才能使用服务器的任何能力。

有两种方式可以对 MCP 服务器的访问进行认证：一是通过 [Laravel Sanctum](/docs/{{version}}/sanctum) 或任何通过 `Authorization` HTTP 标头传递的令牌进行简单的基于令牌的认证；二是使用 [Laravel Passport](/docs/{{version}}/passport) 通过 OAuth 进行认证。

<a name="oauth"></a>
### OAuth 2.1

保护基于 Web 的 MCP 服务器最健壮的方式，是使用 [Laravel Passport](/docs/{{version}}/passport) 进行 OAuth 认证。

通过 OAuth 对 MCP 服务器进行认证时，需在 `routes/ai.php` 文件中调用 `Mcp::oauthRoutes` 方法，注册所需的 OAuth2 发现和客户端注册路由。然后，在 `routes/ai.php` 文件中为 `Mcp::web` 路由应用 Passport 的 `auth:api` 中间件：

```php
use App\Mcp\Servers\WeatherExample;
use Laravel\Mcp\Facades\Mcp;

Mcp::oauthRoutes();

Mcp::web('/mcp/weather', WeatherExample::class)
    ->middleware('auth:api');
```

#### 全新安装 Passport

如果你的应用尚未使用 Laravel Passport，请按照 Passport 的[安装与部署指南](/docs/{{version}}/passport#installation)将其添加到应用中。在继续之前，你应已拥有 `OAuthenticatable` 模型、新的认证守卫以及 Passport 密钥。

接下来，应发布 Laravel MCP 提供的 Passport 授权视图：

```shell
php artisan vendor:publish --tag=mcp-views
```

然后，使用 `Passport::authorizationView` 方法指示 Passport 使用该视图。通常，该方法应在应用的 `AppServiceProvider` 的 `boot` 方法中调用：

```php
use Laravel\Passport\Passport;

/**
 * 引导任何应用服务。
 */
public function boot(): void
{
    Passport::authorizationView(function ($parameters) {
        return view('mcp.authorize', $parameters);
    });
}
```

在认证过程中，该视图会展示给最终用户，供其拒绝或批准 AI 代理的认证请求。

![Authorization screen example](data:image/png;base64,)

> [!NOTE]
> 在此场景中，我们只是将 OAuth 用作底层可认证模型的转换层。我们忽略了 OAuth 的许多方面，例如作用域（scope）。

#### 使用现有的 Passport 安装

如果你的应用已经在使用 Laravel Passport，Laravel MCP 应能在你现有的 Passport 安装中无缝工作。但由于 OAuth 主要用作底层可认证模型的转换层，目前不支持自定义作用域。

Laravel MCP 通过上文讨论的 `Mcp::oauthRoutes` 方法，添加、通告并使用单个 `mcp:use` 作用域。

#### Passport 与 Sanctum 的选择

OAuth 2.1 是模型上下文协议规范中记载的认证机制，也是在 MCP 客户端中支持最广泛的方式。因此，我们建议尽可能使用 Passport。

如果你的应用已经在使用 [Sanctum](/docs/{{version}}/sanctum)，那么添加 Passport 可能会比较繁琐。这种情况下，我们建议在出现明确且必要的需求（需要使用仅支持 OAuth 的 MCP 客户端）之前，继续使用 Sanctum 而不引入 Passport。

<a name="sanctum"></a>
### Sanctum

如果你想使用 [Sanctum](/docs/{{version}}/sanctum) 保护 MCP 服务器，只需在 `routes/ai.php` 文件中为服务器添加 Sanctum 的认证中间件。然后，确保你的 MCP 客户端提供 `Authorization: Bearer <token>` 标头，以保证认证成功：

```php
use App\Mcp\Servers\WeatherExample;
use Laravel\Mcp\Facades\Mcp;

Mcp::web('/mcp/demo', WeatherExample::class)
    ->middleware('auth:sanctum');
```

<a name="custom-mcp-authentication"></a>
#### 自定义 MCP 认证

如果你的应用签发自己的自定义 API 令牌，你可以通过为 `Mcp::web` 路由分配任意所需的中间件来对 MCP 服务器进行认证。你的自定义中间件可以手动检查 `Authorization` 标头，对传入的 MCP 请求进行认证。

<a name="authorization"></a>
## 授权

你可以通过 `$request->user()` 方法访问当前认证用户，从而在 MCP 工具和资源中执行[授权检查](/docs/{{version}}/authorization)：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * 处理工具请求。
 */
public function handle(Request $request): Response
{
    if (! $request->user()->can('read-weather')) {
        return Response::error('Permission denied.');
    }

    // ...
}
```

<a name="testing-servers"></a>
## 测试服务器

你可以使用内置的 MCP Inspector 或通过编写单元测试来测试 MCP 服务器。

<a name="mcp-inspector"></a>
### MCP Inspector

[MCP Inspector](https://modelcontextprotocol.io/docs/tools/inspector) 是一个用于测试和调试 MCP 服务器的交互式工具。使用它可以连接到你的服务器、验证身份认证，并试用工具、资源和提示词。

你可以为任何已注册的服务器运行 inspector：

```shell
# Web 服务器...
php artisan mcp:inspector mcp/weather

# 名为 "weather" 的本地服务器...
php artisan mcp:inspector weather
```

该命令会启动 MCP Inspector，并提供客户端设置，你可以将其复制到你的 MCP 客户端中，以确保一切配置正确。如果你的 Web 服务器受认证中间件保护，连接时请务必包含所需的标头，例如 `Authorization` Bearer 令牌。

<a name="unit-tests"></a>
### 单元测试

你可以为 MCP 服务器、工具、资源和提示词编写单元测试。

首先，创建一个新的测试用例，并在注册该原语的服务器上调用它。例如，要测试 `WeatherServer` 上的一个工具：

```php tab=Pest
test('tool', function () {
    $response = WeatherServer::tool(CurrentWeatherTool::class, [
        'location' => 'New York City',
        'units' => 'fahrenheit',
    ]);

    $response
        ->assertOk()
        ->assertSee('The current weather in New York City is 72°F and sunny.');
});
```

```php tab=PHPUnit
/**
 * 测试一个工具。
 */
public function test_tool(): void
{
    $response = WeatherServer::tool(CurrentWeatherTool::class, [
        'location' => 'New York City',
        'units' => 'fahrenheit',
    ]);

    $response
        ->assertOk()
        ->assertSee('The current weather in New York City is 72°F and sunny.');
}
```

类似地，你可以测试提示词和资源：

```php
$response = WeatherServer::prompt(...);
$response = WeatherServer::resource(...);
```

你还可以在调用原语之前链式调用 `actingAs` 方法，以认证用户的身份执行：

```php
$response = WeatherServer::actingAs($user)->tool(...);
```

收到响应后，你可以使用各种断言方法来验证响应的内容和状态。

你可以使用 `assertOk` 方法断言响应成功。它会检查响应是否没有任何错误：

```php
$response->assertOk();
```

你可以使用 `assertSee` 方法断言响应包含特定文本：

```php
$response->assertSee('The current weather in New York City is 72°F and sunny.');
```

你可以使用 `assertHasErrors` 方法断言响应包含错误：

```php
$response->assertHasErrors();

$response->assertHasErrors([
    'Something went wrong.',
]);
```

你可以使用 `assertHasNoErrors` 方法断言响应不包含错误：

```php
$response->assertHasNoErrors();
```

你可以使用 `assertName()`、`assertTitle()` 和 `assertDescription()` 方法断言响应包含特定的元数据：

```php
$response->assertName('current-weather');
$response->assertTitle('Current Weather Tool');
$response->assertDescription('Fetches the current weather forecast for a specified location.');
```

你可以使用 `assertSentNotification` 和 `assertNotificationCount` 方法断言已发送通知：

```php
$response->assertSentNotification('processing/progress', [
    'step' => 1,
    'total' => 5,
]);

$response->assertSentNotification('processing/progress', [
    'step' => 2,
    'total' => 5,
]);

$response->assertNotificationCount(5);
```

最后，如果你想检查原始响应内容，可以使用 `dd` 或 `dump` 方法输出响应，以便调试：

```php
$response->dd();
$response->dump();
```
