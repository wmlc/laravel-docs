...
如需对单个事件进行深入调试，请查阅 [Laravel Telescope](/topic/Laravel%2013.x/e296opq9q7.html)。
...
> [!NOTE]
> 如果你不希望将 Pulse 数据存储在应用的主数据库中，可以指定专用的数据库连接。
...
> [!NOTE]
> 你可以通过实现 `Laravel\Pulse\Contracts\ResolvesUsers` 契约并将其绑定到 Laravel 的[服务容器](/topic/Laravel%2013.x/x3vo054vm1.html)中，来完全自定义已认证用户的捕获与获取方式。

### 卡片

#### 服务器

`<livewire:pulse.servers />` 卡片展示所有运行 `pulse:check` 命令的服务器的系统资源使用情况。有关系统资源上报的更多信息，请参阅 servers 记录器 文档。
...
要了解如何自定义 Pulse 获取与展示用户信息的方式，请参阅我们关于解析用户的文档。

> [!NOTE]
> 如果你的应用收到大量请求或派发大量任务，你可能希望启用采样。详情请参阅 user requests 记录器、user jobs 记录器 和 slow jobs 记录器 文档。

#### 异常

`<livewire:pulse.exceptions />` 卡片展示应用中发生的异常出现的频率与最近发生时间。默认情况下，异常会根据异常类及其发生位置进行分组。详情请参阅 exceptions 记录器 文档。

#### 队列

`<livewire:pulse.queues />` 卡片展示应用中队列的吞吐量，包括已入队、处理中、已处理、已释放和已失败的任务数量。详情请参阅 queues 记录器 文档。

#### 缓慢请求

`<livewire:pulse.slow-requests />` 卡片展示应用中超出配置阈值的传入请求，该阈值默认是 1,000ms。详情请参阅 slow requests 记录器 文档。

#### 缓慢任务

`<livewire:pulse.slow-jobs />` 卡片展示应用中超出配置阈值的排队任务，该阈值默认是 1,000ms。详情请参阅 slow jobs 记录器 文档。
...
详情请参阅 slow queries 记录器 文档。

#### 缓慢的出站请求

`<livewire:pulse.slow-outgoing-requests />` 卡片展示使用 Laravel [HTTP 客户端](/topic/Laravel%2013.x/dgy7x15vw2.html) 发起、且超出配置阈值的出站请求，该阈值默认是 1,000ms。

默认情况下，条目会按完整 URL 分组。不过，你可能希望使用正则表达式对相似的出站请求进行归一化或分组。详情请参阅 slow outgoing requests 记录器 文档。

#### 缓存

`<livewire:pulse.cache />` 卡片展示应用的缓存命中与未命中统计，包括全局和单个键的统计数据。

默认情况下，条目会按键分组。不过，你可能希望使用正则表达式对相似的键进行归一化或分组。详情请参阅 cache interactions 记录器 文档。

## 捕获条目

大多数 Pulse 记录器会根据 Laravel 派发的框架事件自动捕获条目。不过，servers 记录器 和一些第三方卡片必须定期轮询信息。要使用这些卡片，你必须在所有应用服务器上运行 `pulse:check` 守护进程：
...
> [!NOTE]
> Pulse 使用[缓存](/topic/Laravel%2013.x/5dve2w3v4x.html)存储重启信号，因此在使用此功能前，你应确认应用已正确配置了缓存驱动。

### 记录器

记录器负责从应用中捕获条目并记录到 Pulse 数据库。记录器在Pulse 配置文件的 `recorders` 部分注册和配置。

#### 缓存交互

`CacheInteractions` 记录器捕获应用中发生的[缓存](/topic/Laravel%2013.x/5dve2w3v4x.html)命中与未命中信息，用于在 Cache 卡片上展示。
...
`Exceptions` 记录器捕获应用中可报告的异常信息，用于在 Exceptions 卡片上展示。

你可以选择性地调整采样率和忽略的异常模式。你还可以配置是否捕获异常发生的位置。捕获的位置会显示在 Pulse 仪表盘上，有助于定位异常来源；不过，如果同一个异常发生在多个位置，则每个不同位置都会出现一次。

#### 队列

`Queues` 记录器捕获应用队列的信息，用于在 Queues 卡片上展示。

你可以选择性地调整采样率和忽略的任务模式。

#### 缓慢任务

`SlowJobs` 记录器捕获应用中缓慢任务的信息，用于在 Slow Jobs 卡片上展示。

你可以选择性地调整缓慢任务阈值、采样率和忽略的任务模式。
...
`SlowOutgoingRequests` 记录器捕获使用 Laravel [HTTP 客户端](/topic/Laravel%2013.x/dgy7x15vw2.html) 发起、且超出配置阈值的出站 HTTP 请求信息，用于在 Slow Outgoing Requests 卡片上展示。

你可以选择性地调整缓慢出站请求阈值、采样率和忽略的 URL 模式。
...
`SlowQueries` 记录器捕获应用中任何超出配置阈值的数据库查询，用于在 Slow Queries 卡片上展示。

你可以选择性地调整缓慢查询阈值、采样率和忽略的查询模式。你还可以配置是否捕获查询位置。捕获的位置会显示在 Pulse 仪表盘上，有助于定位查询来源；不过，如果同一个查询在多个位置执行，则每个不同位置都会出现一次。
...
`Requests` 记录器捕获向应用发起请求的信息，用于在 Slow Requests 和 Application Usage 卡片上展示。

你可以选择性地调整缓慢路由阈值、采样率和忽略的路径。
...
`Servers` 记录器捕获为应用提供算力的服务器的 CPU、内存和存储使用情况，用于在 Servers 卡片上展示。该记录器要求在你希望监控的每台服务器上运行 pulse:check 命令。
...
`UserJobs` 记录器捕获应用中派发任务的用户信息，用于在 Application Usage 卡片上展示。

你可以选择性地调整采样率和忽略的任务模式。

#### 用户请求

`UserRequests` 记录器捕获向应用发起请求的用户信息，用于在 Application Usage 卡片上展示。

你可以选择性地调整采样率和忽略的 URL 模式。

### 过滤

如前所述，许多记录器都提供通过配置根据条目的值（例如请求的 URL）来"忽略"传入条目的能力。但有时根据其他因素（例如当前已认证的用户）过滤记录会很有用。要过滤这些记录，你可以向 Pulse 的 `filter` 方法传入一个闭包。通常，`filter` 方法应在应用 `AppServiceProvider` 的 `boot` 方法中调用：
...
你可以通过设置 `PULSE_DB_CONNECTION` 环境变量来自定义 Pulse 使用的[数据库连接](/topic/Laravel%2013.x/kl9no87vz4.html)。
...
默认情况下，在 HTTP 响应发送给客户端或任务处理完成后，Pulse 会将条目直接存储到配置的数据库连接；不过，你可以使用 Pulse 的 Redis 摄取驱动将条目发送到 Redis 流。可以通过配置 `PULSE_INGEST_DRIVER` 环境变量来启用：

```ini
PULSE_INGEST_DRIVER=redis
```

默认情况下，Pulse 会使用你默认的[Redis 连接](/topic/Laravel%2013.x/569x518yep.html)，但你可以通过 `PULSE_REDIS_CONNECTION` 环境变量自定义：
...
你可以改为在某些 Pulse 数据记录器上启用"采样"。例如，在 User Requests 记录器上将采样率设为 `0.1`，意味着你只记录约 10% 的访问你应用的请求。在仪表盘中，数值会被放大并加上 `~` 前缀，表示它们为近似值。

一般来说，某个指标拥有的条目越多，你就能在不过多牺牲准确性的情况下将采样率设得越低。

### 修剪

一旦存储的条目超出仪表盘时间窗口，Pulse 会自动修剪它们。修剪在摄取数据时发生，采用可通过 Pulse 配置文件自定义的抽奖机制。
...
定义好 Livewire 组件和模板后，该卡片就可以被引入到你的仪表盘视图中：
...
如果你的自定义卡片位于应用代码库中，并且你正在使用 Laravel 的 [Vite 集成](/topic/Laravel%2013.x/ndvm3gj93j.html)，你可以更新 `vite.config.js` 文件，为卡片引入一个专用的 CSS 入口点：
...
然后，你可以在仪表盘视图中使用 `@vite` Blade 指令，并指定卡片的 CSS 入口：
...
> [!NOTE]
> 在构建捕获当前已认证用户 ID 的卡片扩展包时，你应使用 `Pulse::resolveAuthenticatedUserId()` 方法，该方法会遵循对应用所做的任何用户解析器自定义。
